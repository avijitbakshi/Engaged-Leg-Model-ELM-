% ORIGINAL NAME accln_betainterceptfix_activeleg_notby_com.m
function accl=accln_parabolic_RL_vtannedswitch(rcomub,v ,ank2ank,swmuscl,c,mub,flag,RKcall);%sw=switch
...
facclnoiseap=80;facclnoiseml=80;facclnoisez=0;
fconstbiasnoise=0.1;
...
if RKcall == 1 %No change in beta for Rkcall>1
          if COM_dependent_activeleg==1
              ...
              legcutoff=55/100;flagcom=flag;
                    ...
                    else flagactiveleg=0;%%No eng/diseng 
                    end
          end
end
...
if flagactiveleg == 1 %left
                    ...
                    rcomlt= rcomub-anklt;rcomfromengagedank=rcomlt;hcomlt=norm(rcomlt);ncaplt= rcomlt./hcomlt; %vector from lt leg ankle to com
                               if newmus==1 taumusub2 =muscle2RL(rcomub,v,flagactiveleg,alphatauap,ftaumusnoise,noiseonoffswitch);taumusub=taumusub2;end
...
sign_tauplate2flap_lt_rt=[1 -1];
                                if flagactivelegx==1 %left +y
                                    tauplate2ftml=-alphatauml*rcomu(1);% 1  proj = ap here (diff from expt 2)
                                     tauplate2ftap=UPFRONTSIGN_AFTER_TAUENGCODE_UNDSTOOD(1)*alphatauap*abs(rcomu(1));

                                elseif flagactivelegx==-1
                                    tauplate2ftml=-alphatauml*rcomu(1);% 1  proj = ap here (diff from expt 2)
                                    tauplate2ftap=UPFRONTSIGN_AFTER_TAUENGCODE_UNDSTOOD(2)*alphatauap*abs(rcomu(1));
...
betaengagedlegload=slopebeta*absyproj+0.5;%see beta.xls sheet2
elseif swcori==1 & ktmgt4==1

betaengagedlegload=slopebeta*absyproj+0.5;%%ON DAY OF THESIS I TRY CORIOLIS
...
sign_tauplate2flap_lt_rt=[1 -1];
                                if flagactivelegx==1 %left +y
                                    tauplate2ftml=-alphatauml*rcomu(1);% 1  proj = ap here (diff from expt 2)
                                    if v(2)>0 constantAPtorqueRL=1;else constantAPtorqueRL=0;end
