...
slopetaurange=0:50:1000;slopebetarange=0:1:10;


APmusclegoalbound=0.04;%NOISE SOURCE %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
MLmusclebound=0.02;%NOISE SOURCE %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
APgoalthresh=APmusclegoalbound;% APgoalthresh=0.05;%NOISE SOURCE %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
swmlV=0;% for edgeloopcontrol
edgeloopcontrol=0;%make it less pointy turn, more loopy
midVcontrol=0;
APswgoal=1;%so not stay in mid AP region
APedgestiffens=1;% so not fall off in AP edges
MLedgestiffens=0;

%SIMULATION SETTINGS
tstop=20;
centerborder =0.01/4;0.004/2;% centerborder =0.00005;
iterruns=10;
...
sync=1;syncdisp=500;forcedisplaylim=2000;%May 30 Organised so may 26 comment invalid %26 May 2014 moved up , commented below
shortperiod=20;
...
kkx=1;                                                                                                                                                                                                                                                                                                                 
kky=1/3;
hcomub=0.9;%2/3*1.5;%ht wrt to center of ankles
if randinitials==0
...
for iterrun=1:iterend
    iterrun               
                    ...
                    if randinitials==1
                        xlean0=(rand-0.5)/10; %-0.05*kkx;
                        ylean0=(2*rand-1)*.02;%-0.05*kky;
                        z0=sqrt(hcomub^2-xlean0^2-ylean0^2);
                        rcomub0=[xlean0 ylean0 z0];
...
brake=0;
            ...
            a=accln_Pre_rotation_Coriolis_off(rcomo,vo ,ank2ank,swm,c,mub,flag0,1);
            disply=0;
            k1 = h *a;
            l1 = h *vo;
            k2 = h *accln_Pre_rotation_Coriolis_off( rcomo+ l1./2, vo + k1./2,ank2ank,swm,c,mub,flag0,2);
            l2 = h *(vo + k1./2 );
            k3 = h *accln_Pre_rotation_Coriolis_off( rcomo+ l2./2, vo + k2./2,ank2ank,swm,c,mub,flag0,3);
            l3 = h *(vo + k2./2 );
            ...
            k4 = h *accln_Pre_rotation_Coriolis_off(rcomo+ l3, vo + k3,ank2ank,swm,c,mub,flag0,4);
            disply2=0;
            l4 = h *(vo + k3 );
...
if swmlV
            ...
            if ~correcting
                    ...
                    vMLtrigthresh=0.02;%rand/usetogetbigvthresh;
...
br(ktm)=brake;
            ...
            if plotrealtime
                    ...
                    if length(PARAMRANGE1)==1 extratit=['   ' 'ktm:' num2str(ktm)];
                     ...
                     hold on
                    lsr=size(sr,1);
                    colour=[0 1 0];%if ktm*dt> tstop/2 colour=[0 0 1];end
                               
                    if ktm==pauseordisplay_interval plot(rcomub0(2),rcomub0(1),'k*');end
                    plot(sr(lsr,2),sr(lsr,1),'ro','MarkerSize',2)
                                
                            if ~onceplot       
                         title(['K_{S}= ' num2str(alphatauml) '       K_{ED}= ' num2str(slopebeta) '   C:on' extratit]);
...
if length(PARAMRANGE1)>1
    ...
    pth=cd;
    pathfig=[pth '\CurrentFigs2save\'];
    else
    pathfig='/Users/abakshi/Desktop/work/BRISHTI ANALYSIS DATA/3 MODELING2 twoleg/CurrentFigs2save/';
    end
    saveas(hP,[pathfig 'P.fig'])
    saveas(hT,[pathfig 'T.fig'])
    hvar=figure(1300);
    surf(PARAMRANGE2,PARAMRANGE1,var_t_fall_matrix)%changed x<-->y in surf
    set(gca,'YTick',[0:tickparam1:PARAMRANGE1(end)])
    set(gca,'XTick',[0:tickparam2:PARAMRANGE2(end)])  
    zlim([0 20])
    zlabel('Variance')
...
hv(1,APvthresh4goal)
 hv(1,-APvthresh4goal)
 ibr1=find(br==1)';
 plot(ibr1,sv(ibr1,1),'mo')
 plot(ibr1,sr(ibr1,1),'co')
end