...
slopetau2range=[0:2:20];slopebetarange=[0:20];slopetau1range=[0:100:2000];
...
v0=[0.08*0 vr*cos(theta) -vr*sin(theta)];
...
a=accln_RL_1g(rcomo,vo ,ank2ank,swm,c,mub,flag0,1);
disply=0;
k1 = h *a;
l1 = h *vo;
k2 = h *accln_RL_1g( rcomo+ l1./2, vo + k1./2,ank2ank,swm,c,mub,flag0,2);
l2 = h *(vo + k1./2 );
k3 = h *accln_RL_1g( rcomo+ l2./2, vo + k2./2,ank2ank,swm,c,mub,flag0,3);
l3 = h *(vo + k2./2 );
k4 = h *accln_RL_1g( rcomo+ l3, vo + k3,ank2ank,swm,c,mub,flag0,4);
l4 = h *(vo + k3 );
...
if mod(ktm,pauseordisplay_interval)==0 
...
set(gcf,'position',[ 1330          88         560         420])
    ...
    figure(2000+ks1row);BLACK_and_WHITE_KS_KED_and_Error;
    saveas(gcf,[num2str( fix(gravity/(9.8*1.2))+1) 'g ' get(gcf,'name') '  a2a=' stra2a 'cm' '  ' ],'fig')   
close(2000+ks1row)
...
for ifg=1:lk2
    figure(7000+ifg);BLACK_and_WHITE_KS_KED_and_Error;view([-270 -90])
    saveas(gcf,[num2str( fix(gravity/(9.8*1.2))+1) 'g ' get(gcf,'name') '  a2a=' stra2a 'cm' '  ' endtime],'fig')
end
